...
type InstanceConsumptionStatus struct {
	...
	ReservationAnalysis string `json:"reservation_analysis,omitempty"`
}
...
func ParseTime(dateStr string) (time.Time, bool) {
	...
	switch formatUsed {
	case "Jan 2":
		// Case: User gave "Month Day". Infer the year.
		// If the parsed date is after today's date, it's likely for the previous year.
		// Example: If today is Jan 5, 2024 and user enters "Dec 31", it should be Dec 31, 2023.
	...
	case "02":
		...
		// If the parsed day is after today's day, assume it's for the previous month.
...
func (h *PlainHandler) Handle(ctx context.Context, r slog.Record) error {
	...
	if h.opts.AddSource && r.PC != 0 {
		fs := runtime.CallersFrames([]uintptr{r.PC}) //nolint:staticcheck
		f, _ := fs.Next()
...
func CheckInstanceConsumptionCore(ctx context.Context, req CheckConsumptionRequestShared, defaultProjectID string) (InstanceConsumptionStatus, error) {
	...
	req.ReservationName = strings.TrimSpace(req.ReservationName)
	...
	if isSpot {
		...
		case "SPECIFIC_RESERVATION":
			...
			// If a reservation name is passed in, use it. Otherwise, use the one from the affinity.
			resName := req.ReservationName
			if resName == "" {
				resName = val
			}
			status.ConsumptionStatus = fmt.Sprintf("Consuming (Specific Reservation: %s)", resName)
	...
	// Reservation Timestamp Analysis (if applicable)
	if req.StockoutTimestamp != "" && req.ReservationName != "" && !isSpot {
		stockoutTime, err := time.Parse(time.RFC3339, req.StockoutTimestamp)
		if err != nil {
			status.ReservationAnalysis = fmt.Sprintf("Error parsing stockout timestamp: %v", err)
			return status, nil
		}

		// Fetch the specific reservation
		reservation, err := service.Reservations.Get(projectID, status.Zone, req.ReservationName).Context(ctx).Do()
		if err != nil {
			status.ReservationAnalysis = fmt.Sprintf("Could not fetch reservation '%s': %v", req.ReservationName, err)
			return status, nil
		}

		// Parse reservation timestamps
		startTime, errStart := time.Parse(time.RFC3339, reservation.CreationTimestamp)
		// Reservations don't have a fixed EndTime in the v1 API, using a very far future date if not applicable
		_ = time.Now().AddDate(100, 0, 0)

		if errStart != nil {
			status.ReservationAnalysis = "Could not parse reservation creation timestamp."
			return status, nil
		}

		// Perform the check
		if stockoutTime.After(startTime) {
			status.ReservationAnalysis = fmt.Sprintf("VALID: Stockout at %s occurred AFTER the reservation was created (%s). This indicates a potential capacity issue despite the reservation being active.",
				stockoutTime.Format(time.Kitchen), startTime.Format(time.Kitchen))
		} else {
			status.ReservationAnalysis = fmt.Sprintf("INVALID: Stockout at %s occurred BEFORE the reservation was created (%s). The stockout was expected as the reservation was not yet active.",
				stockoutTime.Format(time.Kitchen), startTime.Format(time.Kitchen))
		}
	}
...
func ProcessConsumptionRequest(ctx context.Context, instanceNames []string, zone, reqProjectID, stockoutTimestamp, reservationName, defaultProjectID string) (string, error) {
	...
	for _, name := range instanceNames {
		sharedReq := CheckConsumptionRequestShared{
			...
			StockoutTimestamp: stockoutTimestamp,
...
func SearchStockoutLogsMCP(ctx context.Context, projectID string, req SearchStockoutLogsRequest, filter_type string) (string, error) {
	...
	if filter_type == "gke" {
		...
		return "", fmt.Errorf("invalid filter_type for SearchStockoutLogsMCP: %s", filter_type)
	}

	// If ClusterNames are provided, add them to the filter.
	// If the slice is empty, this block is skipped, and the search applies to all clusters.
	if len(req.ClusterNames) > 0 {
		var clusterFilters []string
		for _, name := range req.ClusterNames {
			clusterFilters = append(clusterFilters, fmt.Sprintf(`resource.labels.cluster_name="%s"`, name))
		}
		filter += fmt.Sprintf(" AND (%s)", strings.Join(clusterFilters, " OR "))
	}
	...
	endDate, endValid := ParseTime(req.EndDate)
	if startValid {
		filter += fmt.Sprintf(` AND timestamp >= "%s"`, startDate.Format(time.RFC3339))
	} else {
		// Default to 7 days if no date provided
		defaultStart := time.Now().AddDate(0, 0, -14)
		filter += fmt.Sprintf(` AND timestamp >= "%s"`, defaultStart.Format(time.RFC3339))
	}
	if endValid {
		filter += fmt.Sprintf(` AND timestamp <= "%s"`, endDate.Format(time.RFC3339))
	}
...
func getInstanceNameFromLogEntry(entry *logging.Entry) string {
	// Priority 1: Check GCE-specific resource labels first.
	if v, ok := entry.Labels["compute.googleapis.com/resource_name"]; ok && v != "" {
		return v
	}

	// Priority 2: Check common resource labels used by GKE and other services.
	// "instance_name" is often used for the VM name.
	// "node_name" is what GKE uses for the k8s node, which is the VM name.
	// "instance_id" is a fallback, but it's often the numeric ID.
	labels := []string{"instance_name", "node_name", "instance_id", "name"}
